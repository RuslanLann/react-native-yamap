...
@implementation RNYMView {
    ...
    NSNumber* userLocationIconScale;
...
-(void) setCenter:(YMKPoint*) center withZoom:(float) zoom {
    YMKCameraPosition* cameraPosition = [YMKCameraPosition cameraPositionWithTarget:center zoom:zoom azimuth:0 tilt:0];
    YMKAnimation* animationType = [YMKAnimation animationWithType:YMKAnimationTypeSmooth duration:0.4];
    [self.mapWindow.map moveWithCameraPosition:cameraPosition animationType:animationType cameraCallback:nil];
...
-(void) updateUserIcon {
    if (userLocationView != nil && userLocationImage) {
        ...
        [iconStyle setScale:userLocationIconScale];
        [userLocationView.pin setIconStyleWithStyle:iconStyle];
        [userLocationView.arrow setIconStyleWithStyle:iconStyle];
    }
}
...
- (void)insertReactSubview:(UIView<RCTComponent>*)subview atIndex:(NSInteger)atIndex {
    if ([subview isKindOfClass:[YamapPolygonView class]]) {
        ...
        YMKMapObjectCollection *objects = self.mapWindow.map.mapObjects;
        YamapCircleView* circleView = (YamapCircleView*) subview;
        //    YMKCircleMapObject* obj = [objects addCircleWithCircle:[circle getCircle]];
        YMKCircleMapObject* obj = [circleView addCircleToMapObjects:objects];
        [circleView setMapObject:obj];
    } else {
